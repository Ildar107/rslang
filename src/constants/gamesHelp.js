...
const SPRINT_HELP = [
  `Ход игры: перед вами появляется слово с предполагаемым переводом.
    Перевод может быть как правильным так и неправильным.
    Вам нужно угадать 10 правильно или неправильно переведенных слов за отведенное время.
    В конце раунда выводится статитика набранных очков.
    Есть возможность выбирать значение таймера`,
  'Необходимые действия: кликайте по кнопке "ВЕРНО" или "НЕВЕРНО"',
];
...
const SAVANNAH_HELP = [
  `Ход игры: появляется произвольное слово на английском относящееся к данному уровню.
    Имеется 4 блока с различными вариантами перевода.
    Нужно выбрать слово с правильным переводом пока аглийское слово летит в конечную точку.
    В конце раунда выводится статитика набранных очков`,
  'Необходимые действия: кликайте по переводу который, на ваш взгляд, является верным',
];
...
export {
  AUDIOCALL_HELP, SPRINT_HELP, SPEAKIT_HELP, WORD_BUILDER_HELP, PUZZLE_HELP, SAVANNAH_HELP,
};
